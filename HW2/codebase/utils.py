...
def log_normal(x, m, v):
    ...
    x_flatten = x.view(*x.shape[:-1], x.shape[-1])
    m_flatten = m.view(*m.shape[:-1], m.shape[-1])
    v_flatten = v.view(*v.shape[:-1], v.shape[-1])
    sub= torch.subtract(x_flatten, m_flatten)
    std= torch.sqrt(v_flatten)
    div= torch.div(sub,std)
    inside_exp = torch.mul(torch.square(div), -0.5)
    prod= torch.log(torch.mul(np.sqrt(2*np.pi),std))
    log_probs = inside_exp - prod
    log_normal= log_probs.sum(-1)
    # return log_normal


    # d = x.shape[-1]
    # cov = v_flatten.view(*v_flatten.shape[:-1], d, 1) * \
    #     torch.eye(d, device=x.device).repeat(
    #         *v_flatten.shape[:-1], 1).view(*v_flatten.shape[:-1], d, d)
    # return torch.distributions.MultivariateNormal(m_flatten, covariance_matrix=cov).log_prob(x_flatten)
    result = torch.distributions.Normal(loc=m_flatten,
                                      scale=v_flatten.sqrt()).log_prob(x_flatten).sum(-1)
    
    assert(torch.allclose(result,log_normal,atol=0.1))
    return result
...
def log_normal_mixture(z, m, v):
    ...
    return log_mean_exp(log_probs, 1)


def log_normal_wentao(x, m, v):
    """
    Computes the elem-wise log probability of a Gaussian and then sum over the
    last dim. Basically we're assuming all dims are batch dims except for the
    last dim.
    ...
    m_flatten = m.view(*m.shape[:-1], m.shape[-1])
    v_flatten = v.view(*v.shape[:-1], v.shape[-1])
    return torch.distributions.Normal(loc=m_flatten,
                                      scale=v_flatten.sqrt()).log_prob(x_flatten).sum(-1)

def log_normal_mixture_wentao(z, m, v):
    """
    Computes log probability of Gaussian mixture.
    ...
    """
    # TODO: Implement this
    b, mixtures, d = m.shape
    z_self_repeat = z.repeat(1, mixtures, 1).view(b, mixtures, d)
    log_probs = log_normal(z_self_repeat, m, v)
    return log_mean_exp(log_probs[0], 0)
